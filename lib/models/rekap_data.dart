...
class PADData {
  ...
  double get pajakPersentase =>
      pajakAnggaran == 0 ? 0 : (pajakRealisasi / pajakAnggaran) * 100;

  // Retribusi
  final double retribusiAnggaran;
  final double retribusiRealisasi;
  double get retribusiPersentase => retribusiAnggaran == 0
      ? 0
      : (retribusiRealisasi / retribusiAnggaran) * 100;

  // Pengelolaan Kekayaan
  final double pengelolaanAnggaran;
  final double pengelolaanRealisasi;
  double get pengelolaanPersentase => pengelolaanAnggaran == 0
      ? 0
      : (pengelolaanRealisasi / pengelolaanAnggaran) * 100;

  // Lain-lain PAD
  final double lainPadAnggaran;
  final double lainPadRealisasi;
  double get lainPadPersentase =>
      lainPadAnggaran == 0 ? 0 : (lainPadRealisasi / lainPadAnggaran) * 100;

  // Total PAD
  double get totalAnggaran =>
      pajakAnggaran + retribusiAnggaran + pengelolaanAnggaran + lainPadAnggaran;
  double get totalRealisasi =>
      pajakRealisasi +
      retribusiRealisasi +
      pengelolaanRealisasi +
      lainPadRealisasi;
  double get totalPersentase =>
      totalAnggaran == 0 ? 0 : (totalRealisasi / totalAnggaran) * 100;
  ...
  factory PADData.fromRow(List<dynamic> row, int tahun) {
    ...
    // [8]: RETRIBUSI %
    // [9]: PENGELOLAAN KEKAYAAN ANGGARAN
    // [10]: PENGELOLAAN KEKAYAAN REALISASI
    // [11]: PENGELOLAAN KEKAYAAN %
    // [12]: LAIN PAD ANGGARAN
    // [13]: LAIN PAD REALISASI
    // [14]: LAIN PAD %
    // [15]: TOTAL ANGGARAN (optional, we calculate ourselves)
    // [16]: TOTAL REALISASI (optional, we calculate ourselves)
    // [17]: TOTAL % (optional)
  ...
  bool get isProvinsi =>
      nomorUrut.startsWith('X') ||
      nomorUrut.startsWith('I') && nomorUrut.length <= 4;
...
class DaerahDataGroup {
  ...
  double get rataRataPAD {
    ...
    return dataPerTahun.map((d) => d.totalRealisasi).reduce((a, b) => a + b) /
        dataPerTahun.length;
  }
  ...
  double get rataRataPajak {
    if (dataPerTahun.isEmpty) return 0;
    return dataPerTahun.map((d) => d.pajakRealisasi).reduce((a, b) => a + b) /
        dataPerTahun.length;
  }
}
